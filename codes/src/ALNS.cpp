...
ALNS::ALNS()
{
  this->num_elements_in_pool_ = 0;
  this->pool_ = std::vector<ALNSHeuristicSolution*>(K_SIZE_OF_ALNS_POOL,NULL);
  Reset();
}
...
void ALNS::Reset()
{
  ...
  this->num_elements_in_pool_ = 0;
  this->pos_best_sol_ = -1;
  this->pos_worst_sol_ = -1;
  this->last_improve_iteration_ = 0;
  this->non_improve_iterations_counter_ = 0;
}

bool ALNS::CheckPoolIntegrity()
{
  //(this->mutex_).lock();
  ALNSHeuristicSolution * curr_sol = NULL;
  int max_profits = -1, min_profits = std::numeric_limits<int>::max();
  int max_profits_index = -1, min_profits_index = -1;
  if(this->num_elements_in_pool_ > K_SIZE_OF_ALNS_POOL) throw "Inconsistent pool 1";
  //if(this->num_elements_in_pool_ != (int)((this->pool_).size())) throw "Inconsistent pool 2";

  for(int i  = 0; i < this->num_elements_in_pool_; ++i)
  {
    curr_sol = (this->pool_)[i];
    if(!(curr_sol->CheckCorrectness(*(this->curr_instance_)))) throw "Inconsistent pool 3";

    if(curr_sol->profits_sum_ > max_profits)
    {
  ...
  if( ((this->pool_)[max_profits_index])->profits_sum_ != ((this->pool_)[this->pos_best_sol_])->profits_sum_) throw "Inconsistent pool 4";
  if( ((this->pool_)[min_profits_index])->profits_sum_ != ((this->pool_)[this->pos_worst_sol_])->profits_sum_) throw "Inconsistent pool 5";
...
void ALNS::Init(Instance& instance, std::string algo, std::string folder, std::string file_name)
{
  ...
  int num_vertices = graph->num_vertices(), num_arcs = graph->num_arcs(), num_mandatory = graph->num_mandatory();
  ...
  this->ordered_profits_ = std::vector<std::pair<int,int>>(num_vertices - num_mandatory - 2);
  for(int i = num_mandatory + 1; i < num_vertices - 1; ++i) (this->ordered_profits_)[i - num_mandatory - 1] = std::pair<int,int>(i,(graph->profits())[i]);
  std::sort((this->ordered_profits_).begin(), (this->ordered_profits_).end(), order_less);

  /*for(size_t i = 0; i < (this->ordered_profits_).size(); ++i)
  {
    std::cout << "(" << ((this->ordered_profits_)[i]).first << "," << ((this->ordered_profits_)[i]).second << ")[" << (graph->profits())[((this->ordered_profits_)[i]).first] << "]" << std::endl;
  }*/
...
void ALNS::PrintPool()
{
  ...
  std::cout << "pos best: " << this->pos_best_sol_ << " pos worst: " << this->pos_worst_sol_ << std::endl;
  for(size_t i = 0; i < num_elements_in_pool_; ++i)
  {
    std::cout << (this->pool_)[i]->profits_sum_ << " ";
  }
  std::cout << std::endl;
  (this->mutex_).unlock();
}
...
{
    if(this->pos_best_sol_ != -1) return (this->pool_)[this->pos_best_sol_];
    return NULL;
}

ALNSHeuristicSolution * ALNS::worst_solution()
{
    if(this->pos_worst_sol_ != -1) return (this->pool_)[this->pos_worst_sol_];
    return NULL;
}
...
{
  ALNSHeuristicSolution * curr_sol = NULL;
  if(K_SIZE_OF_ALNS_POOL <= 0) return false;

  int pos_second_worst_sol = -1;
  bool sol_is_in_pool = false;

  sol->BuildBitset(*(this->curr_instance_));

  if((sol->unvisited_vertices_).empty()) sol->is_optimal_ = true;

  if(this->num_elements_in_pool_ == 0)
  {
      (this->pool_)[this->num_elements_in_pool_] = sol;
      (this->num_elements_in_pool_)++;
      this->pos_best_sol_ = 0;
      this->pos_worst_sol_ = 0;
      this->last_improve_iteration_ = iter;
  }else
  {
    // check if sol is already in pool
    sol_is_in_pool = false;
    pos_second_worst_sol = -1;
    if(this->num_elements_in_pool_ > 1)
      pos_second_worst_sol = (this->pos_worst_sol_ + 1) % (this->num_elements_in_pool_);
    ...
    for(int i = 0; i < this->num_elements_in_pool_; ++i)
    {
      ...
      if( (this->num_elements_in_pool_ > 1) && (i != this->pos_worst_sol_) && (curr_sol->profits_sum_ < ((this->pool_)[pos_second_worst_sol])->profits_sum_))
        pos_second_worst_sol = i;
    }

    if(!sol_is_in_pool)
    {
      if(this->num_elements_in_pool_ < K_SIZE_OF_ALNS_POOL)
      {
          (this->pool_)[this->num_elements_in_pool_] = sol;
          (this->num_elements_in_pool_)++;
          if( sol->profits_sum_ > ((this->pool_)[this->pos_best_sol_])->profits_sum_)
          {
              this->pos_best_sol_ =  this->num_elements_in_pool_ - 1;
              this->last_improve_iteration_ = iter;
              //std::cout << "*Melhorou!!!" << std::endl;
          }

          if( sol->profits_sum_ < ((this->pool_)[this->pos_worst_sol_])->profits_sum_)
          {
              this->pos_worst_sol_ = this->num_elements_in_pool_ - 1;
          }
      }else if(this->num_elements_in_pool_ == K_SIZE_OF_ALNS_POOL)
      {
        ...
        if( sol->profits_sum_ > ((this->pool_)[this->pos_worst_sol_])->profits_sum_ )
        {
          if(sol->profits_sum_ > ((this->pool_)[this->pos_best_sol_])->profits_sum_)
          {
              this->pos_best_sol_ =  this->pos_worst_sol_;
              this->last_improve_iteration_ = iter;
              //std::cout << "*Melhorou!!!" << std::endl;
          }

          delete ((this->pool_)[this->pos_worst_sol_]);
          (this->pool_)[this->pos_worst_sol_] = sol;

          // updates pos of worst solution
          if(sol->profits_sum_ > ((this->pool_)[pos_second_worst_sol])->profits_sum_) this->pos_worst_sol_ = pos_second_worst_sol;
        }else
        {
            delete sol;
            //CheckPoolIntegrity();
...
{
  ALNSHeuristicSolution * curr_sol = NULL, * curr_pool_sol = NULL, * best_sol = sol;
  ...
  (this->mutex_).lock();
  std::vector<ALNSHeuristicSolution*> pool_copy(this->num_elements_in_pool_,NULL);

  for(int i = 0; i < this->num_elements_in_pool_; ++i)
    pool_copy[i] = new ALNSHeuristicSolution((this->pool_)[i]);

    //for(int i = 0; i < this->num_elements_in_pool_; ++i)
      //std::cout << ((this->pool_)[i])->profits_sum_ << " == " << (pool_copy[i])->profits_sum_ << std::endl;
  ...
  for(size_t i = 0; i < pool_copy.size(); ++i)
  {
    //std::cout << " * " << i << std::endl;
    /*(this->mutex_).lock();
    if(i == 0) curr_pool_sol = new ALNSHeuristicSolution(this->best_solution());
    else if(i == 1) curr_pool_sol = new ALNSHeuristicSolution(this->worst_solution());
    else curr_pool_sol = this->CopyRandomSolutionFromPool();
    (this->mutex_).unlock();*/
    ...
    if(double_less(curr_similarity,K_SIMILARITY_THRESHOLD))
    {
      ...
      if(curr_sol->profits_sum_ > best_sol->profits_sum_)
      {
            if(best_sol != sol) delete best_sol;
            best_sol = curr_sol;
      }else
      {
        delete curr_sol;
  ...
  if(best_sol != sol) return best_sol;
  return NULL;
}
...
ALNSHeuristicSolution * ALNS::DoPathRelinkingIter(ALNSHeuristicSolution * guiding_sol, ALNSHeuristicSolution * new_sol)
{
    ...
    size_t num_routes = (size_t)((this->curr_instance_)->num_vehicles());
    int num_vertices = graph->num_vertices();
    int num_mandatory = graph->num_mandatory();
    std::list<int> infeasible_routes;
    int curr_vertex = -1;
    double route_limit = (this->curr_instance_)->limit();
    ...
    for(int i = 1; i < num_vertices -1; ++i)
    {
        ...
        {
            ordered_candidate_vertices.push_back(std::pair<int,int>(i,(graph->profits())[i]));
            //std::cout << i << " ";
        }
    }
    //std::cout << std::endl;
    ...
    while(i < size)
    {
        ...
        {
            if(type == 0) curr_vertex = ((ordered_candidate_vertices)[size - 1 - i]).first;
            if(type == 1) curr_vertex = ((ordered_candidate_vertices)[i]).first;
            ...
            if(!(curr_status->selected_))
            {
                ...
                for(int curr_route = 0; curr_route < num_routes; ++curr_route)
                {
                    ...
                    if(!double_greater(((curr_sol->routes_vec_)[curr_route]).time_,route_limit))
                    {
	                    ...
	                    if(curr_sol->PreviewAddVertexToRouteWithinMinimumDistanceIncrease(*(this->curr_instance_), curr_vertex, curr_route, curr_it, curr_profit_variation, curr_time_variation,true))
	                    {
                ...
                if(can_add)
                {
                    ...
                    if(double_greater(((curr_sol->routes_vec_)[route]).time_,route_limit)) infeasible_routes.push_back(route);
                }
            }
            ++i;
        }
        ...
        //std::cout << *curr_sol << std::endl;
        //!(curr_sol->CheckCorrectness(*(this->curr_instance_)));
        ...
        for(auto it2 = infeasible_routes.begin(); it2 != infeasible_routes.end(); ++it2)
        {
            ...
            {
                if(*it3 > num_mandatory) ordered_vertices_route.push_back(std::pair<int,int>(*it3,(graph->profits())[*it3]));
            }
            ...
            do
            {
                if(curr_sol->PreviewRemoveVertex(*(this->curr_instance_), curr_vertex, curr_profit_variation, curr_time_variation, true))
                {
        ...
        do
        {
          ...
          {
            continue_search = false;
            if(this->DoLocalSearchImprovements(curr_sol))
            {
              continue_search = this->TryToInsertUnvisitedVertices(curr_sol);
            }

            if(this->DoReplacementImprovements(curr_sol))
            {
              continue_search = true;
              this->TryToInsertUnvisitedVertices(curr_sol);
            }
          ...
          if(curr_sol->profits_sum_ > best_sol->profits_sum_)
          {
            ...
            //this->AddSolutionToPool(new ALNSHeuristicSolution(curr_sol),ALNS_iter);
          }
        }while(this->ShiftingAndInsertion(curr_sol));
...
bool ALNS::FoundOptimal()
{
  bool return_value = false;
  (this->mutex_).lock();

  ALNSHeuristicSolution * best = this->best_solution();
  if(best != NULL)
  {
    return_value = best->is_optimal_;
  }
...
{
  if(this->num_elements_in_pool_ == 0) return NULL;
  return new ALNSHeuristicSolution((this->pool_)[rand()%(this->num_elements_in_pool_)]);
}

void ALNS::RunOneThread(int num_thread, int num_iterations)
{
  int iter = 0;
  ALNSHeuristicSolution * curr_sol1 = NULL, *curr_sol2 = NULL, *path_relinking_sol = NULL;
  bool continue_inner_loop = false, converged = false;

  while(!(this->FoundOptimal()) && !converged)
  {
      //std::cout << num_thread << std::endl;
      ++iter;
      (this->mutex_).lock();
      curr_sol1 = this->CopyRandomSolutionFromPool();
      (this->mutex_).unlock();
      this->RemoveVerticesFromSolution(curr_sol1, K_ALNS_PERTURBATION_PERCENTAGE);

      do
      {
          /*this->DoLocalSearchImprovements(curr_sol1);
          this->TryToInsertUnvisitedVertices(curr_sol1);
          //std::cout << "1" << std::endl;
          //std::cout << *curr_sol1 << std::endl;
          //std::cout << "- " << curr_sol1->profits_sum_ << std::endl;
          if(this->DoReplacementImprovements(curr_sol1))
          {
          ...
          {
            continue_search = false;
            if(this->DoLocalSearchImprovements(curr_sol1))
            {
              //std::cout << *curr_sol << std::endl;
              continue_search = this->TryToInsertUnvisitedVertices(curr_sol1);
            }
          ...
          (this->mutex_).lock();
          if(curr_sol1->profits_sum_ > (this->best_solution())->profits_sum_)
          {
            this->AddSolutionToPool(new ALNSHeuristicSolution(curr_sol1),iter);
          }
          (this->mutex_).unlock();
      }while(this->ShiftingAndInsertion(curr_sol1));

      /*do
      {
          this->DoLocalSearchImprovements(curr_sol1);

          curr_sol2 = new ALNSHeuristicSolution(curr_sol1);
          this->TryToInsertUnvisitedVertices(curr_sol2);

          if(curr_sol2->profits_sum_ > (this->best_solution())->profits_sum_)
          {
            //std::cout << "funfou" << std::endl;
            (this->mutex_).lock();
            this->AddSolutionToPool(curr_sol2,iter);
            (this->mutex_).unlock();
          }else delete curr_sol2;
      }while(this->ShiftingAndInsertion(curr_sol1));*/
      /*do
      {
        do{
          ...
          if(this->DoLocalSearchImprovements(curr_sol1))
          {
            continue_inner_loop = this->TryToInsertUnvisitedVertices(curr_sol1);
          }
      ...
      if(K_PATH_RELINKING)
      {
        if(this->non_improve_iterations_counter_ >= K_NON_IMPROVE_ITERATIONS_LIMIT)
        {

          if(K_STOP_AT_FIRST_PR_DETECTION) converged = true;
          else
          {
            //std::cout << " RODOU PR" << std::endl;
            this->non_improve_iterations_counter_ = 0;
            path_relinking_sol = this->DoPathRelinking(curr_sol1);

            if(path_relinking_sol != NULL)
            {
                /*if(path_relinking_sol->profits_sum_ > (this->best_solution())->profits_sum_)
                {
                    std::cout << "PR serviu!" << std::endl;
                    //getchar(); getchar();
                }*/

                (this->mutex_).lock();
                this->AddSolutionToPool(path_relinking_sol,iter);
                (this->mutex_).unlock();
            }
          }
        }
      ...
      if(iter >= num_iterations) converged = true;

      (this->mutex_).lock();
      this->AddSolutionToPool(curr_sol1,iter);

      if(this->last_improve_iteration_ != iter) (this->non_improve_iterations_counter_)++;
      else this->non_improve_iterations_counter_ = 0;

      (this->mutex_).unlock();

  }

  //std::cout << "* " << this->best_solution()->profits_sum_ << std::endl;
}
...
void ALNS::Run()
{
  ...
  if(!(curr_sol->is_infeasible_)&&(curr_sol->is_feasible_))
  {
    ...
    do
    {
        ...
        {
          //std::cout << *curr_sol << std::endl;
          continue_search = this->TryToInsertUnvisitedVertices(curr_sol,0);
        }
    ...
    //std::cout << curr_sol->profits_sum_ << std::endl;
    curr_sol->BuildBitset(*(this->curr_instance_));
    //std::cout << curr_sol->profits_sum_ << " ";

    //if(curr_sol->CheckCorrectness(*(this->curr_instance_)) == false){ std::cout << "deu merda" << std::endl; getchar(); getchar();}
    ...
    if((K_ALNS_MULTI_THREAD)&&(num_cores > 1))
    {
      ...
      this->RunOneThread(num_cores-1,iterations_per_thread);

      for(size_t i = 0; i < v.size(); ++i)
      {
          (v[i]).join();
      }
    }else this->RunOneThread(0,K_NUM_ITERATIONS_ALNS);
    ...
    curr_sol->last_improve_iteration_ = this->last_improve_iteration_;
    curr_sol->num_iterations_ = iter;
    curr_sol->elapsed_time_ = timer->CurrentElapsedTime(ti);
    //std::cout << *curr_sol << std::endl;
    //if(curr_sol->CheckCorrectness(*(this->curr_instance_)) == false){ std::cout << "deu merda" << std::endl; getchar(); getchar();}
    //std::cout << curr_sol->profits_sum_ << " " << std::endl;
    //std::cout << "last iter improvement: " << this->last_improve_iteration_ << std::endl;
    //(this->pool_)[this->pos_best_sol_]->WriteSolutionFile(this->curr_instance_->curr_demand(),"teste_improved");
...
ALNSHeuristicSolution* ALNS::BuildBestSolutionFromPool()
{
    ...
    Graph * graph = (this->curr_instance_)->graph();
    int num_vehicles = (this->curr_instance_)->num_vehicles();
    int num_vertices = graph->num_vertices();
    ...
    ALNSHeuristicSolution * new_solution = new ALNSHeuristicSolution(num_vertices,num_arcs,num_vehicles);
  	...
  	IloNumVarArray h(env, (this->num_elements_in_pool_)*num_vehicles, 0.0, 1.0, ILOINT);
    ...
    for(int i = 0; i < this->num_elements_in_pool_; ++i)
    {
        ...
        {
            //std::cout << j << std::endl;
            curr_route = &((((this->pool_)[i])->routes_vec_)[j]);
    ...
    for(int i = 1; i < num_vertices-1; ++i)
    {
        //std::cout << i << std::endl;
        IloExpr exp2(env);
        for(auto it = (vertex_to_route_map[i]).begin(); it != (vertex_to_route_map[i]).end(); ++it) exp2 += h[*it];

        if(i <= num_mandatory) model.add(exp2 == 1);
        else model.add(exp2 <= 1);
        exp2.end();
    }
...
ALNSHeuristicSolution* ALNS::BuildBestSolutionFromGraphInducedByPool()
{
    ...
    for(int i = 0; i < num_vertices; ++i) profits[i] = (graph->profits())[i];
    ...
    for(int i = 0; i < this->num_elements_in_pool_; ++i)
    {
        for(int j = 0; j < num_vehicles; ++j)
        {
            ...
            for(auto it = (curr_route->vertices_).begin(); it != (curr_route->vertices_).end(); ++it)
            {
                v1 = v2;
                v2 = *it;
                if((*new_graph)[v1][v2] == NULL)
                {
                  new_graph->AddArc(v1,v2, ((*graph)[v1][v2])->dist());
                }
            }
            v1 = v2;
            v2 = num_vertices - 1;
    ...
    (this->curr_instance_)->set_graph(new_graph);

    CapacitatedSingleCommodity(*(this->curr_instance_),R,Rn,-1,sol,false,false,false,NULL,NULL);
    ...
    (this->curr_instance_)->set_graph(graph);
...
bool ALNS::DoReplacementImprovements(ALNSHeuristicSolution * sol)
{
    ...
    do
    {
        improved = (this->Do_1_1_Unrouted_Improvements(sol) || this->Do_2_1_Unrouted_Improvements(sol));
...
bool ALNS::DoLocalSearchImprovements(ALNSHeuristicSolution * sol)
{
    ...
    do
    {
        improved = this->DoAllInterRoutesImprovements(sol);
        global_improvement = (global_improvement || improved);

        improved = this->DoAllIntraRouteImprovements(sol);
        global_improvement = (global_improvement || improved);
    }while(improved);
...
bool ALNS::DoIntraRouteImprovementOneRoute(ALNSHeuristicSolution * sol, int route)
{
    ...
    do
    {
        improved = sol->Do2OptImprovement(graph,route);
...
bool ALNS::Do_1_1_Unrouted_Improvements(ALNSHeuristicSolution * solution)
{
  ...
  int profit_variation1 = 0, profit_variation2 = 0;
  double time_variation1 = 0.0;
  int pos_i1 = 0, pos_f1 =0;
  int max_pos1 = 0;
  int num_routes = (this->curr_instance_)->num_vehicles();
  int curr_vertex = 0;
  VertexStatus * curr_status = NULL;
  int num_vertices = ((this->curr_instance_)->graph())->num_vertices(), num_mandatory = ((this->curr_instance_)->graph())->num_mandatory();;
  std::list<int>::iterator next_vertex_to_be_replaced_it, first_vertex_to_be_replaced_it;

  int type = rand()%2;
  for(size_t i = 0; i < (this->ordered_profits_).size(); ++i)
  {
    if(type == 0) curr_vertex = ((this->ordered_profits_)[num_vertices - num_mandatory - 3 - i]).first;
    if(type == 1) curr_vertex = ((this->ordered_profits_)[i]).first;
      ...
      //{
        ...
        for(int r1 = 0; r1 < num_routes; ++r1)
        {
          ...
          for(int i = 0; i <= max_pos1; ++i)
          {
            ...
            {
              if(solution->PreviewInterRouteSwapUnrouted(*(this->curr_instance_),r1,pos_i1,pos_f1,it_i1,it_f1,profit_variation1,time_variation1,it_i2,profit_variation2))
              {
...
bool ALNS::Do_1_1_Improvement(ALNSHeuristicSolution * solution, int r1, int r2)
{
  ...
  if(((route1->vertices_).empty()) || ((route2->vertices_).empty())) return false;
  ...
  for(int i = 0; i <= max_pos1; ++i)
  {
    ...
    {
      pos_i2 = pos_f2 = j;
      if(solution->PreviewInterRouteSwap(*(this->curr_instance_),r1,pos_i1,pos_f1,r2,pos_i2,pos_f2,it_i1,it_f1,profit_variation1,time_variation1,it_i2,it_f2,profit_variation2,time_variation2))
      {
        if(double_less(time_variation1+time_variation2,0.0))
        {
...
bool ALNS::Do_1_0_Improvement(ALNSHeuristicSolution * solution, int r1, int r2)
{
  ...
  if((route1->vertices_).empty()) return false;
  ...
  for(auto it_vertex = (route1->vertices_).begin(); it_vertex != (route1->vertices_).end(); ++it_vertex)
  {
    vertex = *it_vertex;
    for(int pos = 0; pos <= max_pos2; ++pos)
    {
      if(solution->PreviewInterRouteMoveVertex(*(this->curr_instance_),vertex,r2,pos,it,profit_variation1,time_variation1,profit_variation2,time_variation2))
      {
        if(double_less(time_variation1+time_variation2,0.0))
        {
...
bool ALNS::Do_2_1_Improvement(ALNSHeuristicSolution * solution, int r1, int r2)
{
  ...
  if( ((route1->vertices_).size() < 2) || ((route2->vertices_).empty())) return false;
...
bool ALNS::ShiftingOneVertex(ALNSHeuristicSolution * sol, int vertex, double & global_variation)
{
  ...
  if(!(sol->PreviewRemoveVertex(*(this->curr_instance_),vertex,profit_variation1,time_variation1))) return false;
  ...
  status->selected_ = false; // suppose vertex is removed!
    ...
    {
  		if(sol->PreviewAddVertexToRouteWithinMinimumDistanceIncrease(*(this->curr_instance_), vertex, curr_route, curr_it, curr_profit_variation, curr_time_variation))
  		{
  			if(double_less(curr_time_variation,time_variation2))
  			{
  				can_add = true;
  				route = curr_route;
  				it = curr_it;
...
{
  int curr_vertex = 0, profit_variation = 0, curr_route = 0;
  ...
  for(size_t i = 0; i < (this->ordered_profits_).size(); ++i)
  {
    ...
    if(!(curr_status->selected_))
    {
      if(solution->PreviewAddVertexWithinMinimumDistanceIncrease(*(this->curr_instance_),curr_vertex,curr_route,vertex_it,profit_variation,time_variation))
      {
...
{
  int curr_vertex = 0, profit_variation = 0;
  int num_vertices = ((this->curr_instance_)->graph())->num_vertices();
  int num_mandatory = ((this->curr_instance_)->graph())->num_mandatory();
  double time_variation = 0.0;
  bool added_vertex = false;
  ...
  for(int i = 0; i < (int)((this->ordered_profits_).size()); ++i)
  {
    if(type == 0) curr_vertex = ((this->ordered_profits_)[num_vertices - num_mandatory - 3 - i]).first;
    if(type == 1) curr_vertex = ((this->ordered_profits_)[i]).first;
    curr_status =  &((solution->vertex_status_vec_)[curr_vertex]);
    ...
    if(!(curr_status->selected_))
    {
      if(solution->PreviewAddVertexToRouteWithinMinimumDistanceIncrease(*(this->curr_instance_),curr_vertex,curr_route,vertex_it,profit_variation,time_variation))
      {
...
bool ALNS::ShiftingAndInsertion(ALNSHeuristicSolution* solution)
{
  ...
  int num_vehicles = (this->curr_instance_)->num_vehicles();
  Route * curr_route = NULL;
  int initial_profits_sum = solution->profits_sum_;
  ...
  for(int i = 0; i < num_vehicles; ++i)
  {
    ...
    {
        shifting = ((this->ShiftingOneVertex(solution,*it,global_variation)) || shifting);
    }

    if(shifting)
    {
        this->TryToInsertUnvisitedVerticesOneRoute(solution,i);
    }
  }

  // Retorna true, se houver ganho global em soma dos tempos das rotas ou aumento do sum of profits após o shifting.
  if((double_less(global_variation, 0.0))||(solution->profits_sum_ - initial_profits_sum > 0)) return true;
  return false;
}
...
void ALNS::RemoveVerticesFromSolution(ALNSHeuristicSolution* sol, double percentage)
{
  ...
  VertexStatus * curr_status = NULL;
  Graph * graph = (this->curr_instance_)->graph();
  int num_vertices = graph->num_vertices(), profit_variation = 0, route = 0;
  double time_variation = 0.0;
  int num_mandatory = graph->num_mandatory();
  ...
  if(type == 0) max_removed = (int)(std::floor(percentage * num_visited_vertices));
  else max_removed = (int)(std::floor(percentage * num_visited_profitable_vertices));

  if(max_removed <= 0) return;
  else num_vertices_to_remove = 1 + rand()%max_removed;
  ...
  do
  {
      switch(type)
      {
          ...
          case 0:
          {
              ...
              if(vertex_position > 0) std::advance(vertex_it,vertex_position);
          ...
          case 1:
          {
            do
            {
                curr_vertex = ((this->ordered_profits_)[sub_cont]).first;
          ...
          case 2:
          {
            do
            {
                curr_vertex = ((this->ordered_profits_)[num_vertices - num_mandatory - 3 - sub_cont]).first;
                curr_status =  &((sol->vertex_status_vec_)[curr_vertex]);
      ...
      if((curr_vertex > num_mandatory)&&(sol->PreviewRemoveVertex(*(this->curr_instance_),curr_vertex, profit_variation, time_variation)))
