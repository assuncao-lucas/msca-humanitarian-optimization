            ...
            }else
            {
                pos_vk = num_vertices - 1;
						...
						do
						{
							...
							else time_variation += curr_arc->distance();
	...
	int& profit_variation1, double& time_variation1, std::list<int>::iterator& it_i2, std::list<int>::iterator& it_f2, int& profit_variation2, double& time_variation2)
{
	...
	if ((pos_i1 > pos_f1) || (pos_i1 > max_pos1) || (pos_f1 > max_pos1)) return false;
	if ((pos_i2 > pos_f2) || (pos_i2 > max_pos2) || (pos_f2 > max_pos2)) return false;

	// ROUTE 1
	//compute pre_vertex
	it_i1 = (route1->vertices_).begin();
	if(pos_i1 == 0) pre_vertex1 = 0;
	else
	{
		if(pos_i1 > 1) std::advance(it_i1,pos_i1-1);
		pre_vertex1 = *it_i1;
		++it_i1;
	}
	...
	for(int i = 0; i <= pos_f1 - pos_i1; i++)
	{
		...
		if(i > 0) intra_route_time_variation1 += ((*graph)[v1][v2])->distance();
		if(i == pos_f1 - pos_i1) end_segment_vertex1 = v2;
		++it_f1;
		v1 = v2;
	}

	//compute pos_vertex
	if(pos_f1 == max_pos1) pos_vertex1 = num_vertices - 1; // at this point, it_f1 can be .end()!
	else pos_vertex1 = *it_f1;
	...
	if(pos_i2 == 0) pre_vertex2 = 0;
	else
	{
		if(pos_i2 > 1) std::advance(it_i2,pos_i2-1);
		pre_vertex2 = *it_i2;
		++it_i2;
	}
	...
	for(int i = 0; i <= pos_f2 - pos_i2; i++)
	{
		...
		if(i > 0) intra_route_time_variation2 += ((*graph)[v1][v2])->distance();
		if(i == pos_f2 - pos_i2) end_segment_vertex2 = v2;
		++it_f2;
		v1 = v2;
	}

	//compute pos_vertex
	if(pos_f2 == max_pos2) pos_vertex2 = num_vertices - 1; // at this point, it_f2 can be .end()!
	else pos_vertex2 = *it_f2;
...
bool HeuristicSolution::PreviewAddVertexToRouteWithinMinimumDistanceIncrease(Instance& inst, int vertex, int route, std::list<int>::iterator& it, int& profit_variation, double& time_variation, bool allow_infeasible_routes)
{
	...
	if(status->selected_) return false;
...
bool HeuristicSolution::PreviewAddVertex(Instance& inst, int vertex, int route, int pos, std::list<int>::iterator& it, int& profit_variation, double& time_variation)
{
	...
	if((!(status->selected_)) && (pre_arc != NULL) && (pos_arc != NULL) && (curr_arc != NULL))
	{
		...
		if(!double_greater(curr_route->time_ + time_variation,inst.limit())) return true;
	}
	return false;
}
...
bool HeuristicSolution::PreviewRemoveVertex(Instance& inst, int vertex, int& profit_variation, double& time_variation, bool allow_infeasible_routes)
{
	...
	if(pos == (curr_route->vertices_).begin()) pre_vertex = 0;
	else
	{
	...
	++pos_vertex_it;
	if(pos_vertex_it == (curr_route->vertices_).end()) pos_vertex = num_vertices -1;
	else pos_vertex = *pos_vertex_it;
	...
	if(new_arc != NULL)
	{
		...
		if(!double_greater(curr_route->time_ + time_variation,inst.limit()) || allow_infeasible_routes) return true;
	}
	return false;
}
...
bool HeuristicSolution::CheckCorrectness(Instance& instance)
{
	...
	if(this->num_routes_ != num_vehicles) throw "Invalid solution 1";

	if(this->is_infeasible_) return true;
	...
	for(int i = 0; i < num_vehicles; i++)
	{
		...
		if(!((curr_route->vertices_).empty()))
		{
			...
			{
					v2 = *it;
					if(visited_vertices[v2]) throw "Invalid solution 2";
					visited_vertices[v2] = 1;

                    if((v2 > 0)&&(v2 <= num_mandatory)) ++count_mandatory;
					curr_status = &((this->vertex_status_vec_)[v2]);
					if((!(curr_status->selected_)) || (it != curr_status->pos_) || (curr_status->route_ != i) ) throw "Invalid solution 3";
					if(v2 > num_mandatory)
					{
		...
		if(double_greater(curr_route->time_,instance.limit())) throw "Invalid solution 7.2";
		if(curr_profits != curr_route->sum_profits_) throw "Invalid solution 8";
	}

	if(total_profits != this->profits_sum_) throw "Invalid solution 9";

	if(visited_vertices.count() != ((size_t)num_vertices - (this->unvisited_vertices_).size())) throw "Invalid solution 10";

	for(auto it = (this->unvisited_vertices_).begin(); it != (this->unvisited_vertices_).end(); ++it)
	{
		curr_status = &((this->vertex_status_vec_)[*it]);
		if((curr_status->selected_) || (curr_status->pos_ != it) || (curr_status->route_ != -1) || visited_vertices[*it]) throw "Invalid solution 11";
	}

    if(count_mandatory != num_mandatory) throw "invalid solution 12";

	if(visited_arcs != this->bitset_arcs_) throw "Invalid solution 13";
    if(visited_vertices != this->bitset_vertices_) throw "Invalid solution 14";
	return true;
}
...
void HeuristicSolution::BuildBitset(Instance& instance)
{
        ...
        (this->bitset_vertices_)[0] = (this->bitset_vertices_)[num_vertices -1] = 1;
		...
		for(int i = 0; i < num_vehicles; i++)
		{
			...
			{
				(this->bitset_arcs_)[graph->pos(0,num_vertices-1)] = 1;
			}
		}
}
...
{
	if(sol.is_infeasible_) out << "[INFEASIBLE]";
	if(sol.is_optimal_) out << "[OPTMAL]";
	out << "[profits_sum: " << sol.profits_sum_ << "]"<< std::endl;
	for(int i = 0; i < sol.num_routes_; ++i) out << ((sol.routes_vec_)[i]);
	out << "unvisited: ";
	for(auto it = (sol.unvisited_vertices_).begin(); it != (sol.unvisited_vertices_).end(); ++it) out << *it << " ";
	out << std::endl;
    return out;
}
...
void HeuristicSolution::WriteToFile(std::string algo, std::string folder, std::string file_name)
{
    ...
    file << std::setprecision(5) << std::fixed;
		...
		file << K_FILE_DELIMITER << std::endl
				...
				<< "Num routes: ";
		if((this->is_infeasible_)||(!(this->is_feasible_))) file << "0" << std::endl;
		else
		{
...
void HeuristicSolution::ReadFromFile(Instance& inst, std::string algo, std::string folder, std::string file_name)
{
    ...
    input.open(path.c_str(),std::fstream::in);
		...
		if(!(input.is_open())) throw "Error opening file 1";
    while(!(input.eof()))
		{
			getline(input,line);
			if(line == K_FILE_DELIMITER)
			{
				...
				if((num_routes == 0)||(num_routes != this->num_routes_)) this->is_feasible_ = false;
				else
				{
		...
		if( !(this->is_infeasible_) && (this->is_feasible_) && (this->unvisited_vertices_).empty() ) this->is_optimal_ = true;
...
void ALNSHeuristicSolution::WriteToFile(std::string algo, std::string folder, std::string file_name)
{
    ...
    else if(this->is_optimal_) file << "STATUS: OPTIMAL" << std::endl;
		else if(this->is_feasible_) file << "STATUS: FEASIBLE" << std::endl;
			else file << "STATUS: POSSIBLY FEASIBLE" << std::endl;
...
void ALNSHeuristicSolution::ReadFromFile(Instance& inst, std::string algo, std::string folder, std::string file_name)
{
    ...
    input.open(path.c_str(),std::fstream::in);
		...
		if(line == "STATUS: FOUND ARC INTEGER FEASIBLE") this->is_feasible_ = true;
...
std::string ALNSHeuristicSolution::GenerateFileName()
{
	...
	if(K_ALNS_MULTI_THREAD) file_name += "_MT";
  if((K_PATH_RELINKING)&&(!K_STOP_AT_FIRST_PR_DETECTION)) file_name += "_PR";
	file_name += ("_ALNS_");
	if(K_STOP_AT_FIRST_PR_DETECTION) file_name += "CONV";
	else file_name += std::to_string(K_NUM_ITERATIONS_ALNS);
