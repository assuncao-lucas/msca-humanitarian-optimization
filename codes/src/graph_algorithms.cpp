...
{	
	if((P.empty()) && (X.empty()) && (R.size() >= 1)) cliques.push_back(R);
	while(!(P.empty()))
	{
		auto it = P.begin();
		...
		{
			if(P.find(*it2) != P.end()) P2.insert(*it2);
			if(X.find(*it2) != X.end()) X2.insert(*it2);
		}
...
{	
	if((P.none()) && (X.none()) && (R.size() >= 1)) cliques.push_back(R);
