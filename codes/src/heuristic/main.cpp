...
void GenerateALNSLatexTable(std::vector<std::string> dirs, bool stop)
{
    ...
    if(stop) output_name = ".//tables//latex//stop_table_ALNS.txt";
    else output_name = ".//tables//latex//table_ALNS.txt";
    ...
    for(size_t k = 0; k < dirs.size(); ++k)
    {
        ...
        for(size_t i = 0; i < instances.size(); ++i)
        {
            ...
            if(s_lb.str() == "-inf") lb = -1.0;
						...
						if(status == "INFEASIBLE") feasible = false;
            ...
            for(size_t j = 0; j < algorithms.size(); j++)
            {
              ...
              for(int seed = 1; seed <= 10 ; ++seed)
              {
                ...
                if(!(double_equals(best_lb,-1)) && !(double_equals(lb,-1)) && (status != "INFEASIBLE") && (status != "POSSIBLYFEASIBLE") && (status !="FAILEDTOFINDAFEASIBLESOLUTION"))
                {
                    ...
                    if(double_greater(lb,best_heuristic_lb)) best_heuristic_lb = lb;
              ...
              if(count > 0) heuristic_lb = avg_lb/(1.0*count);
              else heuristic_lb = -1.0;
    			    ...
    			    }else if((double_equals(best_lb,-1)) || (double_equals(heuristic_lb,-1)))
              {
    		      ...
    		      {
                    if(double_equals(best_lb,heuristic_lb)) curr_primal_gap = 0.0;
                    else curr_primal_gap = (100*(best_lb - heuristic_lb))/best_lb;
                    if(double_equals(best_lb,best_heuristic_lb)) best_primal_gap = 0.0;
                    else best_primal_gap = (100*(best_lb - best_heuristic_lb))/best_lb;
    		      }
...
void GenerateALNSLatexTable2(std::vector<std::string> dirs, bool stop)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//latex//stop_table_ALNS2.txt";
    else output_name = ".//tables//latex//table_ALNS2.txt";
    output.open(output_name.c_str(),std::fstream::out);
        ...
        for(size_t i = 0; i < num_instances; ++i)
        {
            ...
            for(size_t l = 0; l <= 1; ++l)
            {
              ...
              for(size_t j = 0; j < algorithms.size(); j++)
              {
                ...
                if(double_equals(heuristic_lb,-1.0)) output << "& -- & -- ";
                else output << "& " << heuristic_lb << " & " << best_heuristic_lb;
              }
            }
            output << "\\\\" << std::endl;
        }
        if(k != dirs.size() - 1) output << "\\\\" << std::endl;
    }
...
void GenerateALNSCSVTableFridman(std::vector<std::string> dirs, bool stop, int type, int num_executions)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//CSV//stop_table_ALNS_friedman_" + std::string(1,type) + ".csv";
    else output_name = ".//tables//CSV//table_ALNS_friedman_" + std::string(1,type) + ".csv";
    output.open(output_name.c_str(),std::fstream::out);
    ...
    for(size_t k = 0; k < dirs.size(); ++k)
    {
        ...
        for(size_t i = 0; i < instances.size(); ++i)
        {
            ...
            for(size_t j = 0; j < algorithms.size(); j++)
            {
              ...
              switch(type)
              {
                case '1': output << (100.0 - curr_primal_gap); break;
                case '2': output << (100.0 - best_primal_gap); break;
                case '3': output << (5000.0 - avg_max_improve_iter); break;
              }
              if(j < algorithms.size()-1) output << ",";
            }
...
void GenerateCSVTableFeasibilityPumpFriedmanGaps(std::vector<std::string> dirs, bool stop, int num_seeds = 10)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//CSV//stop_table_fp_fried_gaps.csv";
    else output_name = ".//tables//CSV//table_fp_fried_gaps.csv";
    output.open(output_name.c_str(),std::fstream::out);
...
void GenerateFeasibilityPumpPerInstanceResultsCSV(std::vector<std::string> dirs, bool stop, int num_seeds = 10)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//CSV//stop_table_FP_detailed_per_instance.csv";
    else output_name = ".//tables//CSV//table_FP_detailed_per_instance.csv";
    output.open(output_name.c_str(),std::fstream::out);
    ...
    {
      for(size_t j = 1; j <= 4; ++j) output << algorithms[i] << ";";
    }
...
void GenerateLNSPerInstanceResultsCSV(std::vector<std::string> dirs, bool stop, int num_seeds = 10)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//CSV//stop_table_LNS_detailed_per_instance.csv";
    else output_name = ".//tables//CSV//table_LNS_detailed_per_instance.csv";
    output.open(output_name.c_str(),std::fstream::out);
...
void GenerateFeasibilityPumpLatexTable(std::vector<std::string> dirs, bool stop, int num_seeds = 10)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//latex//stop_table_FP.txt";
    else output_name = ".//tables//latex//table_FP.txt";
    output.open(output_name.c_str(),std::fstream::out);
...
void GeneratePrimalAndDualGapsLatexTable(std::vector<std::string> dirs, bool stop)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//latex//stop_table_primal_dual_gaps_ALNS.txt";
    else output_name = ".//tables//latex//table_primal_dual_gaps_ALNS.txt";
    output.open(output_name.c_str(),std::fstream::out);
    ...
    for(size_t j = 0; j < dirs.size(); j++)
    {
        ...
        for(size_t i = 0; i < instances.size(); i++)
        {
            ...
            if(count > 0) root_lb = avg_lb/(1.0*count);
            else root_lb = -1.0;
...
void GenerateHeuristicSolutionImprovementLatexTable(std::vector<std::string> dirs, bool stop)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//latex//stop_table_primal_heuristic_improvements.txt";
    else output_name = ".//tables//latex//table_primal_heuristic_improvements.txt";
    output.open(output_name.c_str(),std::fstream::out);
              ...
              for(size_t j = 0; j < algorithms.size(); j++)
              {
                ...
                for(int seed = 1; seed <= 10; ++seed)
                {
                    ...
                    if((double_equals(bound,-1)) || (status == "INFEASIBLE") || (double_equals(root_lb,-1)))
                    {
                        curr_improvement = -1;
                        feasible = false;
                    }else curr_improvement = (100*(bound-root_lb))/root_lb;
...
//algorithms.push_back("new2_fp_cuts_alpha2_0_p_fixed_10_ALNS_5000");
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//CSV//stop_table_ALNS_correct.csv";
    else output_name = ".//tables//CSV//table_ALNS_correct.csv";
    output.open(output_name.c_str(),std::fstream::out);
    ...
    for(size_t j = 0; j < algorithms.size(); j++)
    {
      for(int i = 1; i <= 4; i++) output << algorithms[j] << ";";
    }
    output << std::endl;
        ...
        for(size_t i = 0; i < instances.size(); i++)
        {
            ...
            for(size_t j = 0; j < algorithms.size(); j++)
            {
              ...
              for(size_t seed = 1; seed <= 10; seed++)
              {
                ...
                if(double_greater(improve_iter,curr_max_improve_iter)) curr_max_improve_iter = improve_iter;
...
void GenerateCSVTableFeasibilityPumpFriedmanIterations(std::vector<std::string> dirs, bool stop, int num_executions = 10)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//CSV//stop_table_fp_fried_pumps.csv";
    else output_name = ".//tables//CSV//table_fp_fried_pumps.csv";
    output.open(output_name.c_str(),std::fstream::out);
...
void GenerateCSVTableFeasibilityPump(std::vector<std::string> dirs, bool stop, int num_executions = 10)
{
    ...
    std::fstream output;
    std::string output_name;
    if(stop) output_name = ".//tables//CSV//stop_table_fp.csv";
    else output_name = ".//tables//CSV//table_fp.csv";
    output.open(output_name.c_str(),std::fstream::out);
    ...
    for(size_t j = 0; j < algorithms.size(); j++)
    {
      for(int i = 1; i <= 5; i++) output << algorithms[j] << ";";
    }
    output << std::endl;
...
void ParseArgumentsAndRun(int argc, char* argv[] )
{
    ...
    while( ( c = getopt_long (argc, argv, "01:2:3", longOpts, NULL) ) != -1 )
    {
        switch(c)
        {
            ...
            case '1':
              ...
              break;
            case '2':
              if(optarg) seed = std::atoi(optarg);
              break;
            case '3':
              solve_ALNS = true;
              break;
    ...
    {
      slack->set_dist(0.0);
    }else (inst.graph())->AddArc(0,num_vertices-1,0.0);
    ...
    if(solve_local_branching)
    {
      ...
      if(K_LOCAL_BRANCHING_SOLVE_ALNS) initial_sol.ReadFromFile(inst, ALNSHeuristicSolution::GenerateFileName() + "_seed_" + std::to_string(seed), folder, file_name);
      else initial_sol.ReadFromFile(inst, FPHeuristicSolution::GenerateFileName() + "_seed_" + std::to_string(seed), folder, file_name);
