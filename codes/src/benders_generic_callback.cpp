...
void FillObjectiveExpressionDualCompactSingleCommodityContinuousSpace(IloExpr& obj, DualVariablesSingleCommodity& dual_vars, IloNumArray& x_values, IloNumArray& y_values, std::optional<IloNum> dual_bound_value, const double* R0, const double* Rn, const Instance& instance, bool combine_feas_op_cuts)
{
  ...
  for (int budget_iter = 0; budget_iter <= budget; ++budget_iter)
  {
    for (int i = 0; i < num_vertices; ++i)
    {
      const auto vertex_origin_info = vertices_info[i];

      for (int j : graph->AdjVerticesOut(i))
      {
        const GArc * arc = (*graph)[i][j];
        const int arc_pos = graph->pos(i,j);
        double coef1 = (route_limit - R0[i] - vertex_origin_info.nominal_service_time_ - arc->distance())*x_values[arc_pos];
        obj += operator*(dual_vars.v_0_[dual_vars.v_0_var_to_index(arc_pos,budget_iter,num_arcs)], coef1);

        if(j > 0)
        {
          const auto vertex_dest_info = vertices_info[j];
          double vertex_dest_deadline = (j <= num_mandatory)? route_limit: round_decimals(vertex_dest_info.profit_/vertex_dest_info.decay_ratio_,2);

          double coef2 = max(route_limit - vertex_dest_deadline,vertex_dest_info.nominal_service_time_ + Rn[j])*x_values[arc_pos];
          obj -= operator*(dual_vars.v_1_[dual_vars.v_1_var_to_index(arc_pos,budget_iter,num_arcs)], coef2);
        }

        if(i > 0)
        {
          double coef3 = (vertex_origin_info.nominal_service_time_ + arc->distance())*x_values[arc_pos];
          obj -= operator*(dual_vars.v_2_[dual_vars.v_2_var_to_index(arc_pos,budget_iter,num_arcs)], coef3);

          if(budget_iter > 0)
          {
            double coef4 = (vertex_origin_info.nominal_service_time_ + vertex_origin_info.dev_service_time_ + arc->distance())*x_values[arc_pos];
...
void SetSolutionStatus2(IloCplex & cplex, Solution<double> & solution, bool solve_relax)
{
...
void optimizeLP2(IloCplex & cplex, IloEnv& env, IloModel& model,const Instance& instance, double total_time_limit, const double * R0, const double * Rn, Solution<double>& solution)
{
  const Graph* graph = instance.graph();
  ...
  if (!cplex.solve())
  {
    //std::cout << "primal " << cplex.getCplexStatus() << std::endl;
    timer->Clock(tf);
  ...
  SetSolutionStatus2(cplex,solution,true);
...
int f_var_to_index2(int arc_pos, int budget, int num_arcs)
{
  return budget * num_arcs + arc_pos;
}

static void PopulateByRowCompactSingleCommodityContinuousSpace2(IloEnv& env, IloModel& model, IloNumVarArray & x, IloNumVarArray & y, IloNumVarArray &f, std::optional<std::reference_wrapper<IloNumArray>> x_values, std::optional<std::reference_wrapper<IloNumArray>> y_values, const double * R0, const double * Rn, const Instance& instance)
{
  const Graph* graph = instance.graph();
  const int num_vertices = graph->num_vertices();
  const int num_arcs = graph->num_arcs();
  ...
  for(int budget_iter = 0; budget_iter <= budget; ++budget_iter)
  {
    for (int i = 0; i < num_vertices; ++i)
    {
      ...
      for (const int& j: graph->AdjVerticesOut(i))
      {
        ...
        x_values.has_value()?
        model.add(f[f_var_to_index2(arc_pos,budget_iter,num_arcs)] <= (lb_coef)*(x_values->get())[arc_pos])
        : model.add(f[f_var_to_index2(arc_pos,budget_iter,num_arcs)] <= operator*(lb_coef, x[arc_pos]));
        ...
        if(j > 0)
        {
          ...
          x_values.has_value()?
          model.add(f[f_var_to_index2(arc_pos,budget_iter,num_arcs)] >= (ub_coef)*(x_values->get())[arc_pos])
          : model.add(f[f_var_to_index2(arc_pos,budget_iter,num_arcs)] >= operator*(ub_coef, x[arc_pos]));
        }
      }
    }
    ...
    for (int i = 1; i < num_vertices; ++i)
    {
      ...
      for (const int& j: graph->AdjVerticesOut(i))
      {
        ...
        IloExpr exp(env);
        exp += f[f_var_to_index2(arc_pos,budget_iter,num_arcs)];

        for (const int& k: graph->AdjVerticesIn(i))
          exp -= f[f_var_to_index2(graph->pos(k,i),budget_iter,num_arcs)];
        ...
        {
          IloExpr exp2(env);
          exp2 += f[f_var_to_index2(arc_pos,budget_iter,num_arcs)];

          for (const int& k: graph->AdjVerticesIn(i))
            exp2 -= f[f_var_to_index2(graph->pos(k,i),budget_iter-1,num_arcs)];
...
void PrimalSubproblemCompactSingleCommodity2(const Instance& inst, IloNumArray& x_values, IloNumArray& y_values, const double * R0, const double * Rn, double time_limit, bool export_model, Solution<double>& solution)
{
  ...
  PopulateByRowCompactSingleCommodityContinuousSpace2(env,model,x,y,f,x_values,y_values,R0,Rn,inst);
  ...
  for(int j = num_mandatory + 1; j < num_vertices; ++j)
  {
    ...
    for (const int&i: graph->AdjVerticesIn(j))
      obj += operator*(vertex_info.decay_ratio_,f[f_var_to_index2(graph->pos(i,j),budget,num_arcs)]);
  }
    ...
    for(int i = 0; i < num_vertices; ++i)
    {
      for(const auto j: graph->AdjVerticesOut(i))
      {
        for (int budget_iter = 0; budget_iter <= budget; ++budget_iter)
        {
          ...
          f[f_var_to_index2(graph->pos(i,j),budget_iter,num_arcs)].setName(strnum);
        }
      }
    }
  ...
  optimizeLP2(cplex,env,model,inst,time_limit, R0, Rn, solution);
...
IloBool SeparateBendersCutSingleCommodity(IloEnv& master_env, IloNumVarArray &x, IloNumVarArray &y, IloNumVar& dual_bound, IloNumArray &x_values, IloNumArray &y_values, IloNum dual_bound_value, IloCplex& worker_cplex, DualVariablesSingleCommodity* dual_vars, const double* R0, const double* Rn, const Instance & instance,  IloObjective& worker_obj, IloExpr& cut_expr, bool combine_feas_op_cuts, Solution<double>& solution)
{
  ...
  if ( status == IloAlgorithm::Unbounded)
  {
    // Solution<double> solution2(instance.graph()->num_vertices());
    // //std::cout << "Dual unbounded!" << std::endl;
    // PrimalSubproblemCompactSingleCommodity2(instance,x_values,y_values,R0,Rn,-1,true,solution2);
    // if(solution2.is_feasible_)
    //   std::cout << "Primal feasible para dual unbounded!" << std::endl;
  ...
  {
    // Solution<double> solution2(instance.graph()->num_vertices());
    // //std::cout << "Dual optimal!" << std::endl;
    // PrimalSubproblemCompactSingleCommodity2(instance,x_values,y_values,R0,Rn,-1,true,solution2);
    ...
    // if(!solution2.is_optimal_)
    //   std::cout << "Primal non optimal and optimal dual " << solution2.lp_ << " x " << new_dual_bound << std::endl;

    // if(!double_equals(solution2.lp_,new_dual_bound))
    //   std::cout << "Primal non optimal with different bounds! " << solution2.lp_ << " x " << new_dual_bound << std::endl;
    //std::cout << status << " " << new_dual_bound << " x " << dual_bound_value << std::endl;
    // if combining feas and opt cuts, it suffices to have a bound greater than zero to imply a violated cut.
    if(!combine_feas_op_cuts && double_greater(dual_bound_value,new_dual_bound) || combine_feas_op_cuts && !double_equals(new_dual_bound,0))
    {
      ...
      // for(IloInt i = 0; i < dual_vars->v_0_.getSize(); ++i)
      // {
      //   IloInt var_id = dual_vars->v_0_[i].getId();
      //   if(dual_vars->ext_ray_coef_.count(var_id) > 0 )
      //     cut_expr += operator*(v_0_values[i],dual_vars->ext_ray_coef_[var_id]);
      // }

      // for(IloInt i = 0; i < dual_vars->v_1_.getSize(); ++i)
      // {
      //   IloInt var_id = dual_vars->v_1_[i].getId();
      //   if(dual_vars->ext_ray_coef_.count(var_id) > 0 )
      //     cut_expr += operator*(v_1_values[i],dual_vars->ext_ray_coef_[var_id]);
      // }

      // for(IloInt i = 0; i < dual_vars->v_2_.getSize(); ++i)
      // {
      //   IloInt var_id = dual_vars->v_2_[i].getId();
      //   if(dual_vars->ext_ray_coef_.count(var_id) > 0 )
      //     cut_expr += operator*(v_2_values[i],dual_vars->ext_ray_coef_[var_id]);
      // }

      // for(IloInt i = 0; i < dual_vars->v_3_.getSize(); ++i)
      // {
      //   IloInt var_id = dual_vars->v_3_[i].getId();
      //   if(dual_vars->ext_ray_coef_.count(var_id) > 0 )
      //     cut_expr += operator*(v_3_values[i],dual_vars->ext_ray_coef_[var_id]);
      // }

      //double value_cut = 0.0;
      ...
      for (int budget_iter = 0; budget_iter <= budget; ++budget_iter)
      {
        for (int i = 0; i < num_vertices; ++i)
        {
          ...
          for (int j : graph->AdjVerticesOut(i))
          {
            const GArc * arc = (*graph)[i][j];
            const int arc_pos = graph->pos(i,j);
            double coef1 = (route_limit - R0[i] - vertex_origin_info.nominal_service_time_ - arc->distance())*v_0_values[dual_vars->v_0_var_to_index(arc_pos,budget_iter,num_arcs)];
            cut_expr += operator*(coef1, x[arc_pos]);
            //value_cut += coef1*solution.initial_sol_->x_values_[arc_pos];
            ...
            if(j > 0)
            {
              ...
              double coef2 = max(route_limit - vertex_dest_deadline,vertex_dest_info.nominal_service_time_ + Rn[j])* v_1_values[dual_vars->v_1_var_to_index(arc_pos,budget_iter,num_arcs)];
              cut_expr -= operator*(coef2, x[arc_pos]);
              //value_cut -= coef2*solution.initial_sol_->x_values_[arc_pos];
            }

            if(i > 0)
            {
              double coef3 = (vertex_origin_info.nominal_service_time_ + arc->distance())*v_2_values[dual_vars->v_2_var_to_index(arc_pos,budget_iter,num_arcs)];
              cut_expr -= operator*(coef3, x[arc_pos]);
              //value_cut -= coef3*solution.initial_sol_->x_values_[arc_pos];

              if(budget_iter > 0)
              {
                double coef4 = (vertex_origin_info.nominal_service_time_ + vertex_origin_info.dev_service_time_ + arc->distance())* v_3_values[dual_vars->v_3_var_to_index(arc_pos,budget_iter,num_arcs)];
                cut_expr -= operator*(coef4, x[arc_pos]);
                //value_cut -= coef4*solution.initial_sol_->x_values_[arc_pos];
              }
            }
          }
        }
      ...
      if(combine_feas_op_cuts)
      {
        ...
        // if(double_greater(solution.initial_sol_->dual_bound_*v_dual_bound,value_cut))
        //   std::cout << " ** violated! " << solution.initial_sol_->dual_bound_*v_dual_bound << " <= " << value_cut << std::endl;
      }else
      {
        cut_expr -= dual_bound;
        // if(double_greater(solution.initial_sol_->dual_bound_,value_cut))
        //   std::cout << " ** violated! " << solution.initial_sol_->dual_bound_ << " <= " << value_cut << std::endl;
      }
  ...
  for (int budget_iter = 0; budget_iter <= budget; ++budget_iter)
  {
    for (int i = 0; i < num_vertices; ++i)
    {
      ...
      for (int j : graph->AdjVerticesOut(i))
      {
        ...
        const int arc_pos = graph->pos(i,j);
        IloExpr exp1(master_env);
        exp1  += operator*(master_vars.x[arc_pos],route_limit - R0[i] - vertex_origin_info.nominal_service_time_ - arc->distance());
        ext_ray_coef_[v_0_[v_0_var_to_index(arc_pos,budget_iter,num_arcs)].getId()] = exp1;

        if(j > 0)
        {
          const auto vertex_dest_info = vertices_info[j];
          double vertex_deadline = (j <= num_mandatory)? route_limit: round_decimals(vertex_dest_info.profit_/vertex_dest_info.decay_ratio_,2);
          IloExpr exp2(master_env);
          exp2 -= operator*(master_vars.x[arc_pos],max(route_limit - vertex_deadline, vertex_dest_info.nominal_service_time_ + Rn[j]));
        ...
        {
          IloExpr exp3(master_env);
          exp3 -= operator*(master_vars.x[arc_pos], vertex_origin_info.nominal_service_time_ + arc->distance());
          ext_ray_coef_[v_2_[v_2_var_to_index(arc_pos,budget_iter,num_arcs)].getId()] = exp3;

          if(budget_iter > 0)
          {
            IloExpr exp4(master_env);
            exp4 -= operator*(master_vars.x[arc_pos], vertex_origin_info.nominal_service_time_ + vertex_origin_info.dev_service_time_ + arc->distance());
