...
template <typename T> Solution<T>::Solution(void)
{
	this->solution_ = NULL;
	this->dimension_ = 0;
...
template <typename T> Solution<T>::Solution(int dimension)
{
...
template <typename T> void Solution<T>::reset()
{
	...
	if(this->dimension_ > 0) this->solution_ = new Matrix<T>(this->dimension_,this->dimension_,0);
	else this->solution_ = NULL;
	this->is_optimal_ = false;
	this->out_of_memory_ = false;
	this->is_feasible_ = true;
	this->lb_ = -std::numeric_limits<double>::infinity();
...
template <typename T> Matrix<T> * Solution<T>::solution()
{
	return this->solution_;
}

template <typename T> int Solution<T>::dimension()
{
	return this->dimension_;
}

template <typename T> void Solution<T>::set_cut_added(int type, bool root)
{
    ((this->mutex_cut_added_vec_)[type]).lock();
	if(!root) ((this->num_cuts_added_)[type])++;
	else ((this->num_cuts_added_lp_)[type])++;
	((this->mutex_cut_added_vec_)[type]).unlock();
}

template <typename T> void Solution<T>::set_cut_found(int type, bool root)
{
    ((this->mutex_cut_found_vec_)[type]).lock();
	if(!root) ((this->num_cuts_found_)[type])++;
	else ((this->num_cuts_found_lp_)[type])++;
	((this->mutex_cut_found_vec_)[type]).unlock();
}

template <typename T> void Solution<T>::increment_elaspsed_time(double time)
{
...
template <typename T> void Solution<T>::write_to_file(std::string algo, std::string folder, std::string file_name)
{
    ...
    if(this->out_of_memory_) file << "STATUS: OUT_OF_MEMORY" << std::endl;
    else if(!(this->is_feasible_)) file << "STATUS: INFEASIBLE" << std::endl;
        else if(this->is_optimal_) file << "STATUS: OPTIMAL" << std::endl;
            else file << "STATUS: FEASIBLE" << std::endl;
    ...
    for(int i = 0; i < K_NUM_TYPES_CALLBACKS ; i++) file << "type " << i << ": " << (this->num_cuts_added_lp_)[i] << "/" << (this->num_cuts_found_lp_)[i] << std::endl;
    file << "# calls to callback: " << this->num_calls_to_callback_ << std::endl;
    file << "# cuts added/found via callback (MILP): " << std::endl;
    for(int i = 0; i < K_NUM_TYPES_CALLBACKS ; i++) file << "type " << i << ": " << (this->num_cuts_added_)[i] << "/" << (this->num_cuts_found_)[i] << std::endl;
